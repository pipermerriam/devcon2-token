...
def test_token_holder_can_respond(chain, survey, token_id, get_response_data):
    assert survey.call().numResponses() == 0
    assert survey.call().responseCounts(7) == 0

    response_txn_hash = survey.transact().respond(7)
...
def test_token_holder_cannot_respond_twice(chain, survey, token_id, get_response_data):
...
def test_token_id_cannot_respond_twice(chain, web3, survey, token, token_id, get_response_data):
    assert survey.call().hasResponded(token_id) is False
    ...
    duplicate_response_txn_hash = survey.transact({'from': web3.eth.accounts[1]}).respond(7)
    chain.wait.for_receipt(duplicate_response_txn_hash)
...
def test_non_token_holder_cannot_respond(chain, web3, token, survey, get_response_data):
    assert survey.call().numResponses() == 0
    assert not token.call().isTokenOwner(web3.eth.accounts[1])

    response_txn_hash = survey.transact({'from': web3.eth.accounts[1]}).respond(7)
    chain.wait.for_receipt(response_txn_hash)
...
def test_cannot_respond_after_survey_close(chain, web3, token_id, survey, get_response_data, set_timestamp):
